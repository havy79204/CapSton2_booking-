...
export const api = {
  ...
  listProductVariants(productId) {
    return apiFetch(`/api/products/${encodeURIComponent(productId)}/variants`)
  },
  ...
  updateProductVariant(productId, variantId, payload) {
    return apiFetch(`/api/products/${encodeURIComponent(productId)}/variants/${encodeURIComponent(variantId)}`, {
      method: 'PATCH',
      body: JSON.stringify(payload)
    })
  },
  ...
  // Time slot availability for booking
  getTimeSlotAvailability({ salonId, dateISO, technicianId, serviceIds = [] } = {}) {
    const qs = new URLSearchParams()
    if (salonId) qs.set('salonId', salonId)
    if (dateISO) qs.set('dateISO', dateISO)
    if (technicianId) qs.set('technicianId', technicianId)
    if (serviceIds.length) qs.set('serviceIds', serviceIds.join(','))
    return apiFetch(`/api/availability/timeslots?${qs.toString()}`)
  },
  ...
  getProfileBookings() {
    return apiFetch('/api/profile/bookings')
  },
  getProfileOrders() {
    return apiFetch('/api/profile/orders')
  },
  ...
  updateAddress(id, payload) {
    return apiFetch(`/api/addresses/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(payload) })
  },
  deleteAddress(id) {
    return apiFetch(`/api/addresses/${encodeURIComponent(id)}`, { method: 'DELETE' })
  },
  setDefaultAddress(id) {
    return apiFetch(`/api/addresses/${encodeURIComponent(id)}/set-default`, { method: 'POST' })
  },
}
export default api
