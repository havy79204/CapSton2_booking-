...
async function findProductImages(productId) {
  const sql = `SELECT ImageId, ProductId, ImageUrl, DisplayOrder, IsPrimary FROM dbo.ProductImages WHERE ProductId=@productId ORDER BY IsPrimary DESC, DisplayOrder ASC`
  try {
...
async function findProductVariants(productId) {
  const sql = `SELECT VariantId, ProductId, VariantName, VariantType, PriceAdjustment, StockQty, ImageUrl, DisplayOrder, IsAvailable FROM dbo.ProductVariants WHERE ProductId=@productId AND IsAvailable=1 ORDER BY DisplayOrder ASC`
  try {
    const r = await query(sql, { productId })
    return r.recordset || []
  } catch {
