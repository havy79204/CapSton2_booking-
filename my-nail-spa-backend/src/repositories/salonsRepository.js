...
async function upsertSalonProfile(salonId, body) {
  ...
  await query(
    `MERGE dbo.SalonProfiles AS t
       ...
       WHEN MATCHED THEN
         UPDATE SET Name=@name, Address=@address, Phone=@phone, Email=@email, Policy=@policy, AvatarImageUrl=@avatarImageUrl, CoverImageUrl=@coverImageUrl, Description=@description, Features=@features, UpdatedAt=SYSUTCDATETIME()
       WHEN NOT MATCHED THEN
         INSERT(SalonId, Name, Address, Phone, Email, Policy, AvatarImageUrl, CoverImageUrl, Description, Features, CreatedAt, UpdatedAt)
         VALUES(@salonId, @name, @address, @phone, @email, @policy, @avatarImageUrl, @coverImageUrl, @description, @features, SYSUTCDATETIME(), SYSUTCDATETIME());`,
    {
      salonId,
      name: body.name ?? null,
