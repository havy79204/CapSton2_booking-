...
const { applyGiftCardForAmount, ensureGiftCardTables } = require('./giftcardService')
...
async function maybeNotifyBookingCreated(booking, { user } = {}) {
  try {
    console.log('maybeNotifyBookingCreated called', { user });
    const userId = user?.id || user?.UserId;
    if (!userId) {
      console.warn('No userId found for notification', { user });
      return; // không có user thì không gửi
    }
    ...
    await createNotification({
      ...
      body: `Your booking at ${salonName} on ${booking.dateISO} at ${booking.timeSlot} has been created.`,
