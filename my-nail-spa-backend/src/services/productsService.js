...
function mapProductVariant(v) {
  return {
    id: v.VariantId,
    name: v.VariantName,
    type: v.VariantType,
    priceAdjustment: v.PriceAdjustment ? Number(v.PriceAdjustment) : 0,
    stockQty: v.StockQty !== null && v.StockQty !== undefined ? Number(v.StockQty) : null,
    imageUrl: v.ImageUrl,
...
async function getProduct(id) {
  ...
  const images = await productsRepo.findProductImages(id)
  product.images = images.map(mapProductImage)
  
  // Fetch variants
  const variants = await productsRepo.findProductVariants(id)
  product.variants = variants.map(mapProductVariant)
