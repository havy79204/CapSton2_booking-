...
export default function OwnerChatPage() {
  ...
  const loadThreads = React.useCallback(async (silent = false) => {
    try {
      ...
      setError('')
      const data = await api.get('/api/owner/chat/threads')
  ...
  const loadMessages = React.useCallback(async (threadId, silent = false) => {
    ...
    try {
      ...
      setError('')
      const data = await api.get(`/api/owner/chat/threads/${encodeURIComponent(threadId)}/messages`)
  ...
  async function handleSend(event) {
    ...
    try {
      setSending(true)
      setError('')
      const data = await api.post(`/api/owner/chat/threads/${encodeURIComponent(activeThreadId)}/messages`, { text })
          ...
          <div className="owner-chatMessagesScroll" ref={messagesWrapRef} onScroll={handleMessagesScroll}>
            ...
            {messages.map((msg, idx) => (
              <React.Fragment key={`${msg.id}-${msg.createdAt || idx}`}>
                ...
                <div className={`owner-chatBubble ${msg.from === 'owner' ? 'owner' : 'customer'}`}>
